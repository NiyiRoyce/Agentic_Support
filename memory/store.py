...
class InMemoryStore(BaseMemoryStore):
    ...
    async def list_user_sessions(
        ...
        sessions = [
            self._sessions[sid] for sid in session_ids[-limit:] if sid in self._sessions
        ]
...
class RedisStore(BaseMemoryStore):
    ...
    async def save_session(self, session: ConversationSession) -> bool:
        ...
        await client.setex(
            f"session:{session.session_id}", self.ttl_seconds, session_data
        )

        # Track user sessions
        if session.user_id:
            await client.sadd(f"user_sessions:{session.user_id}", session.session_id)
            await client.expire(f"user_sessions:{session.user_id}", self.ttl_seconds)
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        if session and session.user_id:
            await client.srem(f"user_sessions:{session.user_id}", session_id)
