...
class MemoryValidator:
    ...
    def validate_session(
        ...
        if len(session.messages) > self.max_session_messages:
            return (
                False,
                f"Session exceeds maximum of {self.max_session_messages} messages",
            )
...
class ContentSanitizer:
    ...
    def sanitize_message(content: str) -> str:
        ...
        sanitized = "".join(
            char for char in sanitized if char >= " " or char in ["\n", "\t"]
        )
...
class SessionHealthChecker:
    ...
    def should_summarize(
        ...
        """
        return len(session.messages) >= message_threshold and session.summary is None
