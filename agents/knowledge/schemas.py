...
class KnowledgeQuery(BaseModel):
    ...
    filters: dict = Field(
        default_factory=dict, description="Filters for knowledge search"
    )
    max_results: int = Field(
        default=5, description="Maximum number of results to retrieve"
    )
...
class KnowledgeResponse(BaseModel):
    ...
    sources: List[KnowledgeSource] = Field(
        default_factory=list, description="Sources used for answer"
    )
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence in answer")
    requires_human: bool = Field(
        default=False, description="Whether question requires human expertise"
    )
