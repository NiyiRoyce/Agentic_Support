...
class KnowledgeEvaluator:
    ...
    def evaluate_answer(
        ...
        overall = (
            relevance * 0.35 + completeness * 0.25 + accuracy * 0.25 + clarity * 0.15
        )
    ...
    def _evaluate_relevance(self, question: str, answer: str) -> float:
        ...
        answer_terms = set(
            word.lower() for word in re.findall(r"\w+", answer) if len(word) > 3
        )
    ...
    def get_aggregate_metrics(self) -> Dict[str, float]:
        ...
        return {
            "avg_relevance": sum(e["metrics"].relevance_score for e in self.evaluations)
            / len(self.evaluations),
            "avg_completeness": sum(
                e["metrics"].completeness_score for e in self.evaluations
            )
            / len(self.evaluations),
            "avg_accuracy": sum(e["metrics"].accuracy_score for e in self.evaluations)
            / len(self.evaluations),
            "avg_clarity": sum(e["metrics"].clarity_score for e in self.evaluations)
            / len(self.evaluations),
            "avg_overall": sum(e["metrics"].overall_score for e in self.evaluations)
            / len(self.evaluations),
    ...
    def get_low_quality_answers(self, threshold: float = 0.6) -> List[Dict]:
        ...
        return [e for e in self.evaluations if e["metrics"].overall_score < threshold]
